...
## Что уже есть (Фаза 1)
- Лента маркета из живого API `GET /api/listings.php` (пагинация, поиск, pull-to-refresh).
- Карточки товара: фото (превью `_t`), цена/«Договорная»/аренда, бейджи ТОП/Новое/Аренда, город.
- Сетевой слой на `async/await` + `URLSession` (сам держит cookie-сессию — пригодится для входа).
- Бренд-тема (зелёная), модели `Codable` зеркалят `_api_item`.

## Дальше (по плану MVP покупателя)
Карточка товара (детально) → вход/регистрация (нужен мобильный токен-эндпоинт на бэкенде,
т.к. write-эндпоинты за CSRF/same-origin) → чат → безопасная сделка → пуши (APNs).
